...
static set<UINT64> memFootPrint;
...
// we suppose cache blk size is 64B
VOID UpdateFootPrint(ADDRINT addr, UINT32 size = 64)
{
    for (ADDRINT addraligned = addr >> 6;(addraligned<<6) < (addr + size);
         addraligned++) {
        if (memFootPrint.count(addraligned) == 0)
            memFootPrint.insert(addraligned);

        if (memFootPrint1B.count(addraligned) == 0)
            memFootPrint1B.insert(addraligned);
...
VOID PrintInfo()
{
    ...
    // show foot print
    UINT64 memFootInc = memFootPrint.size() - memFootLen;
    file_out << "@ footprint current " << memFootPrint.size()*64 << std::endl;
    file_out << "@ footprint increase " << memFootInc*64 << std::endl;
    memFootLen = memFootPrint.size();
...
VOID Fini(int code, VOID* v)
{
    ...
    //out << "# InstCnt: " << instCnter << std::endl;
