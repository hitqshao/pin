...
typedef struct FootPrintInfo {
    ...
    uint64_t memMiss100MHit1B;
    uint64_t memMiss100MHit10B;
    ADDRINT  instAddr;
    ADDRINT  memInstAddr;
    ADDRINT  memDataAddr;
} FootPrintInfo;
...
// footprint of recent 10B inst
static set<UINT64> memFootPrint10B;
...
static ADDRINT instAddr = 0;
static ADDRINT memInstAddr= 0;
static ADDRINT memDataAddr= 0;

static UINT64 memMiss100MHit1B  = 0;
static UINT64 memMiss100MHit10B = 0;
...
{
    ADDRINT addraligned = addr >> 6;
    memDataAddr= addraligned << 6;
    for (;(addraligned<<6) < (addr + size);addraligned++) {
        if (memFootPrint100M.count(addraligned) == 0) {
            memFootPrint100M.insert(addraligned);
        } else if (memFootPrint1B.count(addraligned) != 0) {
            memMiss100MHit1B++;
        } else if (memFootPrint10B.count(addraligned) != 0) {
            memMiss100MHit10B++;
        }
    }
}
...
VOID UpdateFootPrintEvery100M()
{
    for (auto iter = memFootPrint100M.begin();iter != memFootPrint100M.end();iter++) {
        ...
        if (memFootPrint10B.count(*iter) == 0)
            memFootPrint10B.insert(*iter);
    }
}
...
VOID WriteIntoInfoVec() {
    ...
    footPrintInfo.memMiss100MHit1B= memMiss100MHit1B;
    footPrintInfo.memMiss100MHit10B= memMiss100MHit10B;
    footPrintInfo.instAddr = instAddr;
    footPrintInfo.memInstAddr = memInstAddr;
    footPrintInfo.memDataAddr = memDataAddr;
...
VOID PrintInfo(UINT64 instCnter,FootPrintInfo& footInfo)
{
    ...
    file_out << "@ Last Inst Addr: " << std::hex << instAddr << std::endl;
    file_out << "@ Last Mem Inst Addr: " << std::hex << memInstAddr<< std::endl;
    file_out << "@ Last Mem Data Addr: " << std::hex << memDataAddr<< std::endl;
    file_out << "@ Miss 100M Hit 1B: " << std::dec << memMiss100MHit1B<< std::endl;
    file_out << "@ Miss 100M Hit 10B: " << std::dec << memMiss100MHit10B<< std::endl;
...
static void Count100MInst() {
    ...
    // reset mem foot print for 10B
    if (instCnter % 10000000000 == 0) {
        // show foot print
        UINT64 recentFootLen = memFootPrint10B.size();
        file_out << "@ Recent 10B inst footprint " << recentFootLen*64 << std::endl;

        memFootPrint10B.clear();
        memMiss100MHit10B = 0;
    }
